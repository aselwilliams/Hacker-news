...
function App() {
  ...
  const showUserInfo = (username) => {
    ...
    axios.get(`${userUrl}${username}`).then((data) => setUserInfo(data.data));
    setModal(true);
  ...
  const saveToLocalStorage = (news) => {
    localStorage.setItem("hacker-news-searchValue", JSON.stringify(news));
  };
  return (
    <>
      <div className="container">
        ...
        />
        {modal ? (
          <Modal userInfo={userInfo} closeModal={closeModal} />
        ) : (
          <>
            {isLoading ? (
              <LoadingIcon />
